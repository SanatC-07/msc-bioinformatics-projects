...
Original file is located at
    https://colab.research.google.com/drive/1bqO9ag_6xeQ5jbvbtnUMDAfGje03l5ZS
"""
...
def _qc_filter_rnaseq(df_genes, df_meta):
    ...
    Only sample/gene filtering — NO log transform, NO HVG selection.
...
def _qc_filter_microarray(df_genes, df_meta):
    ...
    Only sample filtering — NO log transform, NO HVG selection.
    These steps must happen after train/test split.
    """
...
def get_data(platform, outcome, test_size=0.2, random_state=42, n_top_genes=2000):
    ...
    Correct pipeline (no data leakage):
      1. Load data
      2. QC filtering (sample/gene level) on full data — safe, not outcome-dependent
